       ...
       # ====== 고려대 세종캠퍼스 커스텀 배경 ======
        ...
        for i in range(VIEWPORT_H):
            ...
            pygame.draw.line(
                ...
                (r, g, b),
                (self.scroll * SCALE, i),
                (self.scroll * SCALE + VIEWPORT_W, i),
            )

        # 2. 구름
        cloud_color = (255, 255, 255)
        clouds = [
            (self.scroll * SCALE + 100, 40, 60, 20),
            (self.scroll * SCALE + 280, 25, 80, 18),
            (self.scroll * SCALE + 500, 50, 55, 15),
        ]
        for cx, cy, cw, ch in clouds:
            pygame.draw.ellipse(self.surf, cloud_color, (cx, cy, cw, ch))
            pygame.draw.ellipse(self.surf, cloud_color, (cx + 15, cy - 8, cw - 10, ch))

        # 3. 신정문 석조 아치 실루엣
        gate_color = (180, 170, 155)
        gate_x = int(-self.scroll * SCALE * 0.3 + VIEWPORT_W * 0.6)
        gate_y = VIEWPORT_H - 160

        pygame.draw.rect(self.surf, gate_color, (gate_x - 80, gate_y, 25, 120))
        pygame.draw.rect(self.surf, gate_color, (gate_x + 55, gate_y, 25, 120))
        pygame.draw.rect(self.surf, gate_color, (gate_x - 80, gate_y - 30, 160, 35))
        pygame.draw.ellipse(self.surf, sky_bottom, (gate_x - 55, gate_y - 25, 110, 50))
        for battlement_x in [gate_x - 80, gate_x - 60, gate_x + 45, gate_x + 65]:
            pygame.draw.rect(self.surf, gate_color, (battlement_x, gate_y - 50, 15, 20))

        # 4. 공공정책관 건물 실루엣
        building_color = (80, 85, 95)
        glass_color = (100, 140, 170)
        bx = int(self.scroll * SCALE * 0.05 + VIEWPORT_W * 0.82)
        by = VIEWPORT_H - 200

        pygame.draw.rect(self.surf, building_color, (bx, by, 120, 160))
        for row in range(4):
            pygame.draw.rect(self.surf, glass_color, (bx + 5, by + 15 + row * 35, 110, 20))
        pygame.draw.rect(self.surf, (60, 65, 75), (bx - 20, by + 30, 22, 130))
